...
function ViewTable(props) {
    ...
    const handleSave = async () => {
        ...
        // Проверка на то, что название содержит только кириллицу или пробел, и начинается с буквы
        if (!/^[аАбБвВгГдДеЕёЁжЖзЗиИйЙкКлЛмМнНоОпПрРсСтТуУфФхХцЦчЧшШщЩъЪыЫьЬэЭюЮяЯ]/.test(table.TableName) || /[^аАбБвВгГдДеЕёЁжЖзЗиИйЙкКлЛмМнНоОпПрРсСтТуУфФхХцЦчЧшШщЩъЪыЫьЬэЭюЮяЯ ]/.test(table.TableName))
        {
            // TODO: Выводить ошибку
            console.log("Название должно начинаться с буквы и содержать только кириллицу или пробел");
